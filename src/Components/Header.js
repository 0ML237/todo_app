          ...
          {(toggleMenu || largeur > 500) && (
            <div className="">
              <div className="flex space-x-20 mr-0 border-b-2 border-blue-500">
                {user && <p className="font-sans font-bold flex-col"> <nav>Hello</nav> <nav className="text-blue-500">{user.username}</nav></p>}
                {user && <button className="mt-2 bg-blue-500 rounded-xl text-white font-bold border-current pl-1.5 pr-1.5 pt-0 pb-0 mr-0 max-h-8" onClick={logoutUser}>Logout</button> }
              </div>
              <ul className="liste">
